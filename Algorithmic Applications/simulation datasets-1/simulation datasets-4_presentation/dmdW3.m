...
t = t(:);                  
if length(t) ~= Nt
    warning('Length of t = %d is inconsistent with W4 frame count = %d, auto-interpolated!', length(t), Nt);
    t = linspace(t(1), t(end), Nt)';   
end
dt = mean(diff(t));         
fprintf('Time vector length = %d, average dt = %.6f s\n', length(t), dt);
...
rel_error = norm(W3 - X_dmd, 'fro') / norm(W3, 'fro');
...
for i = 1:K_eff
    ...
    xlabel('Time (s)'); 
    ylabel(sprintf('Mode idx=%d', idx_sel(i)));
    legend('Real part', 'Magnitude', 'Location','best');
    title(sprintf('Energy #%d | f=%.3g Hz | \\sigma=%.2e', i, freq_top(i), sigma_top(i)));
    grid on;
end
...
exportgraphics(gcf, 'dmd_modes_W3_lowfreq_highgrowth_energy_top4.png', 'Resolution', 300);
...
for k = 1:Nt
    ...
    view(-90,30);
    title(sprintf('Original W3, Frame %d / %d', k, Nt));
