...
fprintf('Length of time vector (fixed): %d, average dt = %.4f s\n', length(t), dt);
X1 = W2(:, 1:end-1);       
X2 = W2(:, 2:end);         

%% SVD decomposition 
[U, S, V] = svd(X1, 'econ');
r = 15;                                % Number of retained modes
Ur = U(:, 1:r);
Sr = S(1:r, 1:r);
Vr = V(:, 1:r);
...
rel_error = norm(W2 - X_dmd, 'fro') / norm(W2, 'fro');
...
exportgraphics(gcf, 'dmd_modes_W2_top4.png', 'Resolution', 300);
...
for t = 1:Nt
    ...
    view(-90,30);
    title(sprintf('Original W2, Frame %d / %d', t, Nt));
